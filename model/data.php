<?php 
	...
	class danhba{
	     ...
	     * summary
	     */
	    function __construct($iddb,$idnhan, $tendb, $emaildb, $sdtdb, $ctydb){
	    	$this->iddb    = $iddb;
	    	$this->idnhan  = $idnhan;
	    	$this->tendb   = $tendb;
	    ...
	    static function getListDB(){
	    	...
	    	if($result->num_rows >= 0){
	    		while ($row = $result->fetch_assoc()) {
	    			$db =new danhba($row["maDB"],$row["maNhan"],$row["tenDB"], $row["email"], $row["Sdt"],$row["Congty"]);
	    ...
	    static function addDB($tenadd, $emailadd, $phoneadd,$ctyadd,$nhomadd){
	    	$con = danhba::connect();
			// thao tac voi csdl
	    	$ssuser = $_SESSION["login"];
	    	$sql = "INSERT INTO `danhba` (`maDB`, `maNhan`, `tenDB`, `email`, `Sdt`,`Congty`) VALUES (NULL, '$nhomadd', '$tenadd', '$emailadd', '$phoneadd','$ctyadd')";
	    	$result= $con->query($sql);
